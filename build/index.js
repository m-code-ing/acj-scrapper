...
const clickElement = async (tag, regex, page) => {
...
const clickElementNoNavigation = async (tag, regex, page) => {
    // Find the login button by matching its text content with a regular expression
    const element = await page.$(tag);
    if (element) {
        const buttonText = await element.evaluate((node) => node.textContent);
...
export async function login(url, username, password, chromePath) {
    ...
    try {
        ...
        const nextButtonXpath = '//*[@id="ServiceCommunityTemplate"]/div[2]/div/div[1]/div/div[3]/article/div/div/div[2]/footer/div[2]/button';
        ...
        await buttonElement.click(); // click on the button
    }
    catch (error) {
...
login(url, userData.usename, userData.pwd, "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome");
